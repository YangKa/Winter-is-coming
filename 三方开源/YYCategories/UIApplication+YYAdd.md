...
4.2 在分类里为_YYUIApplicationNetworkIndicatorInfo创建setter和getter方法

```
YYSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo,
                                setNetworkActivityInfo,
                                RETAIN_NONATOMIC,
                                _YYUIApplicationNetworkIndicatorInfo *);
```

4.3 在同步块中修改网络请求数，并开启定时器控制网络指示器延迟显示或延迟消失
